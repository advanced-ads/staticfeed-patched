			...
			if ( $success ) {
				...
				foreach( $Settings as $feed_slug => $feed_settings ) {
					...
					if ( ! file_exists( $local_file ) ) {
						if ( ! @touch( $local_file ) ) {
							$GLOBALS['g_staticfeed_error'] .= sprintf( __( 'Unable to create file %1$s for feed %2$s.' ), $local_file, staticfeed_readable_name( $feed_slug ) );
						} else {
							chmod( $local_file, 0666 );
						}
					} elseif ( ! is_writable( $local_file ) ) {
...
function staticfeed_get_rewrite_rules( $home_root = false ) {
	...
	foreach( $Settings as $feed_slug => $feed_settings ) {
		...
		if ( STATICFEED_DEFAULT_CONTENT_TYPE ) {
			switch ( $feed_slug ) {
				case 'atom':
					$rewrite_content_type .= ',T=application/atom+xml';
					break;
				case 'rdf':
					$rewrite_content_type .= ',T=application/rdf+xml';
					break;
				default:
					$rewrite_content_type .= ',T=' . STATICFEED_DEFAULT_CONTENT_TYPE;
			}
		}
